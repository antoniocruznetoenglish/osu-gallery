...
class SearchEngine(QObject):
    ...
    def sync_fts(self, pattern_id: int) -> None:
        ...
        Rebuilds the searchable text from the pattern's raw_code and its
        ...
        self._upsert_fts(pattern_id, pattern.raw_code, tag_text)
    ...
    def sync_fts_all(self) -> None:
        ...
        for pattern in patterns:
            ...
            tag_text = " ".join(t.name for t in tags)
            self._upsert_fts(pattern.id, pattern.raw_code, tag_text)

    def _upsert_fts(self, pattern_id: int, raw_code: str, tags: str) -> None:
        ...
        self._db.conn.execute(
            f"INSERT INTO {self._fts_table_name} (pattern_id, raw_code, tags) "
            "VALUES (?, ?, ?)",
            (pattern_id, raw_code, tags),
        )
    ...
    def search(self, query: SearchQuery) -> list[Pattern]:
        ...
        Args:
            query: The search query with text, include tags, and exclude tags.

        Returns:
            A list of matching Pattern objects, ordered by relevance.
        """
        if not query.text and not query.tag_ids and not query.exclude_tag_ids:
