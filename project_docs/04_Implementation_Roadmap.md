...
| 2026-07-18 | Bugfix: BUG-121 (FTS sync isolation) + BUG-114 (single-pattern sync threading) | 13_Bugfix_and_Refactor_Backlog.md, search/engine.py, db/database.py, tests/test_search.py | Per-pattern try/except in sync_fts_all() with WARNING logging; _notify_search_sync() changed from logger.debug to logger.warning; pattern_id threaded through _notify_search_sync() for create/update/add/remove/set_pattern ops (sync_fts vs sync_fts_all); added find_missing_fts_entries() diagnostic; diagnostic run against gallery.db found 0 patterns (empty DB, schema not fully migrated); 3 new regression tests (455 total passing) |
| 2026-07-21 | Import usability: drag-drop images, collapsed-text normalizer, singular/plural labels, button rename | 01_Product.md, 04_Implementation_Roadmap.md, 13_Bugfix_and_Refactor_Backlog.md, ui/import_dialog.py, ui/edit_dialog.py, ui/_osu_text_normalizer.py (new), ui/_image_drop_target.py (new), tags/_format_helpers.py (new), tags/mapping_tags.py, ui/thumbnail_widget.py, ui/_preview_pane.py, tests/test_import_text_normalizer.py (new), tests/test_image_drop_target.py (new), tests/test_tag_categories.py, tests/test_ui.py | Task 1: Rename Parse & Save to Save the Pattern (wording-only); Task 2: shared format_object_count() helper with singular/plural for circle/slider/spinner applied to mapping_tags.py, thumbnail_widget.py, _preview_pane.py; Task 3: reusable PySide6 ImageDropTarget widget replacing file-dialog button in both dialogs; Task 4: pure normalize_osu_text() helper restoring Discord-collapsed section headers and hit-object newlines, integrated into import_dialog before parsing; 40+ new tests covering normalizer, drop target, singular/plural formatter, and button rename |
